...
export class WeatherComponent implements OnInit {
  ...
  private transform(id) {
        var code = 'wi-owm-' + id;
        var foundKey = _.findKey(this.openWeatherMapTable, function (o) { return (o === id); });
        if (foundKey) {
            return foundKey;
        }
        else if (this.openWeatherMapTable[code]) {
            return code;
        }
        return null;
    };
  ...
  private refreshWeather() {
    this.apiService.getCurrentWeather()
      .subscribe(weather => {
        ...
        this.weatherIcon = 'wi ' + this.transform(weather.weather[0].id);
